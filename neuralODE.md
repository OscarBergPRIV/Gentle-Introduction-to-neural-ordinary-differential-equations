...
For instance, a simple fully-connected layer or a 2D convolutional layer with a subsequent non-linear activation function can be used.
...
Differences, now the function f can be a learnable parameterized neural layer or a combination of layers, as long as 
...
We will end at the last point $x(t=T)=x_T$.
...
The weights and biases of a NODE-Layer do not only contain the transformative information, which is learned to transform the data from the current to the direct next step (such as in discrete layers),
...
NODE (right) tries to learn the underlying vector field itself. Hence, intermediate data can be spread in a non-uniform manner along the depth-axis, which represents the continuous time. 
...
However, this would lead to high memory costs and additional numerical errors.
...
To solve this integral, we need the trajectory of $a(t)$ and the trajectory of $x(t)$. Since we have a formula for both of them at hand, we can
...
The backward pass can reconstruct the trajectory itself.

Euler`s method is only one type of ODE Solvers, more modern solvers can solve ODEs more efficiently and accurately and have the ability
...
Imagine the input points (red and blue points at the left-hand side) should be transformed along a defined vector space in time to the output points
(right-hand side).
...
Hence, the ResNet can insert some form of desired error in the right step of the trajectories, such that the two trajectories can cross in the transformation space. The "vector field" of a simple ResNet can be "distorted" such that the vectors can cross at the right depth of the network.
